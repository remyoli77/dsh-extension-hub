window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function ExtensionsSection(props) {
      ...
      const filteredMerged = updatableFirst(mergedItems.filter((item) => matchItem(item, qPlugins)))
        ...
        const repoCard = (result) => e('li', { className: 'smCard', key: result.source + ':' + result.fullName },
          e('div', { className: 'smCardRow' },
            ...
            e('div', { className: 'smActions' },
              ...
              }, isBusy('repo:' + result.source + ':' + result.fullName) ? t('installing') : t('installOneClick')),
            ),
          ),
          e('details', {
            className: 'smForm',
        ...
        tab === 'market' ? e('div', { className: 'smTabPane' },
          ...
          e('div', { className: 'smSearchRow' },
            e('input', {
              ...
              'aria-label': t('searchLabel'),
              onChange: (ev) => setQMarket(ev.currentTarget.value),
              onKeyDown: (ev) => {
                if (ev.key === 'Enter') {
                  ev.preventDefault()
                  doSearch()
            ...
            e('button', { type: 'button', className: 'smBtn smBtnPrimary', disabled: isBusy('search') || (source !== 'featured' && qMarket.trim() === ''), onClick: doSearch },
              source === 'featured' ? t('searchFeatured') : t('searchRemote')),
          ),
