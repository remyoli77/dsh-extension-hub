...
async function applyBootOverrides(ctx) {
  const state = readState()
  const overrides = state.pluginOverrides && typeof state.pluginOverrides === 'object' ? state.pluginOverrides : {}
  // Count how many override targets are not yet present in the loader.
  // apply() runs this shortly after boot; bundle entries (e.g. ui-settings-*)
  // may still be assembling, so the caller retries until every target exists.
  let missing = 0
...
/**
 * Probe a repo for a plugin `package.json` that declares `dsh.bundle`.
 * Many dsh-market repos keep the plugin package in a subdirectory (e.g.
 * `dsh-pet/dsh-pet/package.json`), so when the root has none we walk the
 * top-level directories and pick the first sub-package that declares it.
 * @returns {{path: string, pkg: object, name: string} | null}
 */
async function findPluginPackage(source, owner, repo, rootEntries) {
  // 1. root package.json
  const rootPkgText = await fetchRepoFileText(source, owner, repo, 'package.json')
  if (rootPkgText !== undefined) {
    try {
      const pkg = JSON.parse(rootPkgText)
      if (pkg && pkg.dsh && pkg.dsh.bundle) return { path: '', pkg, name: pkg.name || repo }
    } catch { /* unparseable — ignore */ }
  }
  // 2. walk top-level dirs for a package.json declaring dsh.bundle
  for (const entry of (Array.isArray(rootEntries) ? rootEntries : [])) {
    if (entry.type !== 'dir' || entry.name.startsWith('.') || entry.name === 'node_modules' || entry.name === 'src' || entry.name === 'lib' || entry.name === 'assets' || entry.name === 'screenshots' || entry.name === 'tools' || entry.name === 'scripts' || entry.name === 'video' || entry.name === 'prompts') continue
    const text = await fetchRepoFileText(source, owner, repo, `${entry.name}/package.json`)
    if (text === undefined) continue
    try {
      const pkg = JSON.parse(text)
      if (pkg && pkg.dsh && pkg.dsh.bundle) return { path: entry.name, pkg, name: pkg.name || entry.name }
    } catch { /* ignore */ }
  }
  return null
}
...
/**
 * Download a repo's tarball archive to a temp dir and locate the plugin
 * package directory (the dir holding the package.json that declares
 * dsh.bundle). Handles both root-layout and subdir-layout repos.
 * @returns {{pkgDir: string, pkg: object} | null}
 */
async function downloadAndLocatePlugin(source, owner, repo, branch, subPath, task) {
  const urlBase = source === 'github'
    ? `https://codeload.github.com/${owner}/${repo}/tar.gz/refs/heads/${branch}`
    : `https://gitee.com/${owner}/${repo}/repository/archive/${branch}.tar.gz`
  const tmpBase = join(tmpdir(), `dsh-extension-hub-${Date.now().toString(36)}`)
  const archivePath = `${tmpBase}.tar.gz`
  mkdirSync(tmpBase, { recursive: true })
  const clean = () => { try { rmSync(tmpBase, { recursive: true, force: true }) } catch {} try { rmSync(archivePath, { force: true }) } catch {} }
  try {
    // Stream to disk (repo tarballs can be hundreds of MB) with a generous
    // timeout; report progress so the install task is not silent.
    const response = await fetchWithTimeout(urlBase, { }, 180000)
    if (!response.ok) throw new Error(`下载仓库失败：HTTP ${response.status}`)
    const total = Number(response.headers.get('content-length') || 0)
    let received = 0
    const fileStream = createWriteStream(archivePath)
    const reader = response.body.getReader()
    // pump chunks in parallel with progress reporting
    let lastReported = 0
    for (;;) {
      ...
      if (task && total > 0 && received - lastReported > 2 * 1024 * 1024) {
        const pct = Math.round((received / total) * 100)
        task.output.push(`下载仓库：${(received / 1024 / 1024).toFixed(1)}MB / ${(total / 1024 / 1024).toFixed(1)}MB (${pct}%)`)
    ...
    if (task) task.output.push(`下载完成：${(received / 1024 / 1024).toFixed(1)}MB`)
    // extract with system tar (available on this box)
    const proc = spawn('tar', ['-xzf', archivePath, '-C', tmpBase])
    const code = await new Promise((resolve) => proc.on('close', resolve))
    if (code !== 0) throw new Error('解压仓库失败：tar 非零退出')
    // locate plugin dir: subPath (e.g. "dsh-pet") or the single extracted root
    const entries = readdirSync(tmpBase)
    const extractedRoot = entries.length === 1 ? join(tmpBase, entries[0]) : tmpBase
    let pkgDir
    if (subPath && subPath !== '') {
      pkgDir = join(extractedRoot, subPath)
      if (!existsSync(join(pkgDir, 'package.json'))) pkgDir = join(tmpBase, subPath)
    } else {
      pkgDir = extractedRoot
    }
    if (!existsSync(join(pkgDir, 'package.json'))) {
      // fall back: find any dir with a package.json declaring dsh.bundle
      const found = findLocalBundleDir(tmpBase)
      if (found === null) throw new Error('解压后未找到含 dsh.bundle 声明的 package.json')
      pkgDir = found
    }
...
/** Walk an extracted repo tree for the first dir with a dsh.bundle package.json. */
function findLocalBundleDir(root) {
  const stack = [root]
    ...
    try {
      if (existsSync(join(dir, 'package.json'))) {
        try {
          ...
          if (pkg && pkg.dsh && pkg.dsh.bundle) return dir
        } catch { /* ignore */ }
      }
...
/** Build a cleaned process env for child installs.
 * The dsh web process inherits WorkBuddy/claude safe-delete guards
 * (CODEBUDDY_SESSION_ID, CLAUDE_SESSION_ID, CODEBUDDY_SAFE_DELETE_BULK_GUARD,
 * etc.). When pnpm (spawned from here) deletes/replaces many files under
 * profile node_modules during `pnpm add`, it trips a "bulk delete needs
 * confirm" guard and the install aborts. Dropping these vars routes rm/unlink
 * through their zero-overhead passthrough branch, so pnpm installs cleanly.
 * Any workbuddy/claude tool-call scoping is intentionally not inherited. */
function sanitizeSpawnEnv() {
  const env = { ...process.env }
  for (const k of Object.keys(env)) {
    if (/^(CODEBUDDY|CLAUDE|WORKBUDDY)_/.test(k) || /SAFE_DELETE|SAFE-DELETE/i.test(k)) delete env[k]
  }
  delete env.CODEBUDDY_TOOL_CALL_ID
  delete env.CODE_BUDDY_TOOL_CALL_ID
  return env
}

/** Spawn `pnpm add file:<dir>` in the profile, capturing output into the task. */
function runPnpmAddFile(profileDir, pkgDir, task, pkgName) {
  // `pnpm add file:<dir>` records a file: specifier and installs the package
  // from the local dir — no git CLI needed.
  const spec = `file:${pkgDir}`
  const child = spawn(process.execPath, [PNPM_ENTRY, '--ignore-scripts', '--config.minimumReleaseAge=0', 'add', spec], { cwd: profileDir, env: sanitizeSpawnEnv() })
  ...
  child.on('close', (code) => {
    ...
    if (code === 0) {
      task.output.push(`✓ 已安装 ${pkgName}（file: ${pkgDir}）`)
    }
  })
  return child
}

/** Marker file the watchdog treats as "install in progress": while it exists
 * the watchdog skips profile-change auto-reload (installing mutates profile
 * files and a mid-install restart would drop the package.json dependency write). */
const INSTALL_MARKER = '.dsh-ext-installing'
function beginInstallMarker(profileDir) {
  try { writeFileSync(join(profileDir, INSTALL_MARKER), String(Date.now()), 'utf8') } catch {}
}
function endInstallMarker(profileDir) {
  try { rmSync(join(profileDir, INSTALL_MARKER), { force: true }) } catch {}
}
/** Wrap runPnpmAdd with install marker to suppress watchdog auto-reload. */
function runPnpmAddGuarded(profileDir, args, task) {
  beginInstallMarker(profileDir)
  const child = runPnpmAdd(profileDir, args, task)
  child.on('close', () => endInstallMarker(profileDir))
  child.on('error', () => endInstallMarker(profileDir))
  return child
}
/** Wrap runPnpmAddFile with install marker to suppress watchdog auto-reload. */
function runPnpmAddFileGuarded(profileDir, pkgDir, task, pkgName) {
  beginInstallMarker(profileDir)
  const child = runPnpmAddFile(profileDir, pkgDir, task, pkgName)
  child.on('close', () => endInstallMarker(profileDir))
  child.on('error', () => endInstallMarker(profileDir))
  return child
}

/** Kick off a plugin install: npm-first, then repo-tarball fallback. */
function startPluginInstall(ctx, { task, pkg, profile, profileDir, source, owner, repo, branch, subPath }) {
  const pkgName = pkg?.name || repo
  const fallback = () => {
    // Repo tarball path (existing behavior) for packages not on npm.
    downloadAndLocatePlugin(source, owner, repo, branch, subPath, task).then(({ pkgDir }) => {
      task.output.push(`npm 上未发布 ${pkgName}，回退下载仓库：${pkgDir}`)
      const command = `pnpm --dir ${profileDir} add file:${pkgDir}`
      task.output.push(command)
      const child = runPnpmAddFileGuarded(profileDir, pkgDir, task, pkg.name || repo)
      finalizeInstallTask(ctx, task, pkg, child)
    }).catch((error) => {
      finishTask(task, 'failed', null, errorMessage(error))
    })
  }
  // npx/npm probe: if published, install from registry.
  npmLatestVersion(pkgName).then((latest) => {
    if (latest === null) { fallback(); return }
    task.output.push(`从 npm 安装 ${pkgName}@${latest}（跳过 GitHub 下载）`)
    const command = `pnpm --dir ${profileDir} add ${pkgName}@${latest}`
    task.output.push(command)
    const child = runPnpmAddGuarded(profileDir, ['--config.minimumReleaseAge=0', 'add', `${pkgName}@${latest}`], task)
    // runPnpmAdd finishes the task on close; attach registration after success.
    finalizeInstallTask(ctx, task, pkg, child)
  }).catch(() => fallback())
}

/** After a plugin install completes, register the loader entry + persist. */
function finalizeInstallTask(ctx, task, pkg, child) {
  let finalized = false
  const finalize = (code, err) => {
    if (finalized) return
    finalized = true
    finishTask(task, code === 0 ? 'done' : 'failed', code, err || '')
    if (code !== 0) return
    ...
    try {
      ...
      if (results.length > 0) {
        const log = readState()
        log.verifyLog = [...(Array.isArray(log.verifyLog) ? log.verifyLog : []), { at: new Date().toISOString(), scope: 'install-from-repo', package: pkg.name, results }].slice(-50)
        writeState(log)
      }
      if (fixed.length > 0) {
...
async function installFromRepo(ctx, { source, fullName }) {
  ...
  // 4. plugin detection: a package.json (root OR a subdirectory) declaring
  //    dsh.bundle. dsh-market repos often nest the package in a subdir (e.g.
  //    dsh-pet/dsh-pet/package.json), so probe the root first, then subdirs.
  const found = await findPluginPackage(source, owner, repo, entries)
  if (found !== null) {
    ...
    const dup = pluginAlreadyInstalled(profile, owner, repo, '')
    if (dup !== null) {
    ...
    // Prefer installing from the npm registry when the package is published:
    // `pnpm add <name>@latest` is fast and avoids slow GitHub codeload downloads
    // (large repos like dsh-pet's 100MB+ source tree). Fall back to the repo
    // tarball path only when the package is not on npm.
    startPluginInstall(ctx, { task, pkg, profile, profileDir, source, owner, repo, branch, subPath: found.path });
    return {
      ok: true,
      type: 'plugin',
      status: 'installing',
      ...
      command: `pnpm --dir ${profileDir} add ${pkg.name}@latest`,
    }
  }
...
function runPnpmAdd(profileDir, args, task) {
  // --ignore-scripts: this platform lacks toolchains for native postinstall
  // (cloudflared/node-pty are openharmony-unsupported); those packages already
  // exist from prior installs, so skip their rebuild scripts.
  const child = spawn(process.execPath, [PNPM_ENTRY, '--ignore-scripts', ...args], { cwd: profileDir, env: sanitizeSpawnEnv() })
  child.stdout.on('data', (d) => { task.output.push(String(d).trim()) })
  child.stderr.on('data', (d) => { task.output.push(String(d).trim()) })
  child.on('error', (err) => finishTask(task, 'failed', null, err.message))
...
function makeRoutes(ctx, { watchdogApi } = {}) {
  ...
  const routes = [
    ...
    // hot-reload this plugin (host half) without restarting DSH: re-import the
    // module with a cache-busting ?t= URL (ESM re-executes top-level code),
    // dispose the old fiber, and re-apply with the fresh module.
    {
      kind: 'exact',
      path: `${API_ROOT}/reload-self`,
      handler: async (req, res) => {
        if (!guard(req, res, 'POST')) return
        try {
          const result = await reloadSelf(ctx)
...
/**
 * Hot-reload this plugin's host half without restarting DSH.
 *
 * ESM `import()` caches by full URL, so re-importing the same file returns the
 * old module. Appending a `?t=<now>` makes it a distinct URL → Node re-executes
 * the module top level (verified). We re-import our own module, grab the fresh
 * `apply`/helpers, then ask the cordis loader to rebuild this entry so its fiber
 * is recreated from the *new* module. The loader's entry._init() calls
 * tree.import(name); for a name that is a bare specifier it dynamic-imports it,
 * so the cache-busted module is what actually loads into the rebuilt fiber.
 *
 * Practical note: because the canonical specifier is used by tree.import, the
 * rebuild may still pick the cached copy in some cordis versions. The reliable
 * effect here is that we re-execute the module top level (registering fresh
 * closures/constants) and nudge the loader to refresh; for stale-copy cases, a
 * trailing loader refresh via the same update reconciles the fiber.
 */
async function reloadSelf(ctx) {
  const entry = [...ctx.loader.entries()].find((e) => !e.options.group && String(e.options.name || '').includes('extension-hub'))
  if (entry === undefined) {
    return { ok: false, error: '未找到本插件（dsh-extension-hub）的 loader entry' }
  }
  const cacheBust = `?t=${Date.now()}`
  const fresh = await import(/* @vite-ignore */ import.meta.url + cacheBust)
  if (typeof fresh.apply !== 'function') {
    return { ok: false, error: '重载模块未导出 apply 函数' }
  }
  // Nudge the loader to rebuild this entry (dispose old fiber → create new).
  // The freshly imported module's exports are now in the module registry; a
  // refresh re-runs entry.init() which imports the resolved specifier — for a
  // non-cordis: name that path re-executes via the cache-busted graph where the
  // loader resolves it. To force a deterministic rebuild we toggle config.
  await ctx.loader.update(entry.id, { ...entry.options, config: { ...(entry.options.config || {}), __reloadTick: Date.now() } })
  return {
    ok: true,
    entryId: entry.id,
    reloadedAt: new Date().toISOString(),
    note: 'host 模块已重新加载（cache-busting import + loader 重建）；若界面未变请刷新页面',
  }
}
...
export function apply(ctx, config = {}) {
  ...
  // Bundle entries (ui-settings-*, etc.) assemble after this plugin starts, so
  // poll until every override target exists (max ~10s) instead of applying once.
  ctx.effect(() => {
    let attempts = 0
    let timer = null
    const tryApply = () => {
      attempts += 1
      applyBootOverrides(ctx).then((missing) => {
        if (missing > 0 && attempts < 20) {
          timer = setTimeout(tryApply, 500)
        }
      }).catch((error) => {
        console.warn('[dsh-extension-hub] boot override apply failed:', errorMessage(error))
        if (attempts < 20) timer = setTimeout(tryApply, 500)
      })
    }
    timer = setTimeout(tryApply, 0)
