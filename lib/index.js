...
async function fetchRepoFileText(source, owner, repo, path) {
  try {
    if (source === 'github') {
      ...
      const response = await fetch(url, { headers: { ...UA_HEADERS, accept: 'application/vnd.github.raw' } })
...
async function installFromRepo(ctx, { source, fullName }) {
  ...
  for (const path of skillPaths) {
    ...
    if (result.ok) installedSkills += 1
    else skillErrors.push(`${parsed.name}: ${result.error}`)
  }
  if (installedSkills > 0) {
    return { ok: true, type: 'skill', installed: installedSkills, name: repo }
  }
...
function watchdogScriptPath(config) {
  ...
  return fileURLToPath(new URL('../tools/dsh-watchdog.ps1', import.meta.url))
}

/** 枚举正在运行的守护进程 ID（powershell -File ...dsh-watchdog.ps1，排除 -Command 宿主）。 */
...
async function stopWatchdog() {
  ...
  for (const pid of pids) {
    try {
      spawn('taskkill', ['/F', '/PID', String(pid)], { windowsHide: true, stdio: 'ignore' }).unref()
    } catch { /* best effort */ }
  }
  return pids.length
}
...
function makeWatchdogApi(config) {
  ...
  return {
    ...
    async status() {
      ...
      let state = null
      try { state = JSON.parse(readFileSync(join(dshHome, 'dsh-watchdog-state.json'), 'utf8')) } catch { /* absent */ }
      let logTail = []
      try {
    ...
    async start() {
      const running = (await watchdogPids()).length > 0
      if (!running) startWatchdog(scriptPath)
