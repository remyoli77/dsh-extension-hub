...
async function installFromRepo(ctx, { source, fullName }) {
  ...
  if (found !== null) {
    ...
    // Download the tarball, extract, locate the plugin dir (no git CLI needed)
    // and install via `pnpm add file:<dir>` — works for root & subdir layouts.
    downloadAndLocatePlugin(source, owner, repo, branch, found.path, task).then(({ pkgDir }) => {
      const command = `pnpm --dir ${profileDir} add file:${pkgDir}`
      task.output.push(`下载并解压完成：${pkgDir}`)
    ...
    return {
      ...
      command: `pnpm --dir ${profileDir} add file:${found.path || '.'}`,
    }
  }
