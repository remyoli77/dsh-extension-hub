 ...
 */
async function downloadAndLocatePlugin(source, owner, repo, branch, subPath) {
  ...
  try {
    const response = await fetchWithTimeout(urlBase, { }, 60000)
    if (!response.ok) throw new Error(`下载仓库失败：HTTP ${response.status}`)
    const body = await response.arrayBuffer()
    writeFileSync(archivePath, Buffer.from(body))
...
async function installFromRepo(ctx, { source, fullName }) {
  ...
  if (found !== null) {
    ...
    downloadAndLocatePlugin(source, owner, repo, branch, found.path).then(({ pkgDir }) => {
