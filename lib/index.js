...
async function checkUpdates(ctx) {
  ...
  const pluginTasks = Object.entries(deps)
    ...
    .map(async ([name, spec]) => {
      ...
      const gitSpec = specStr.includes('git+') || specStr.startsWith('git:')
...
function startUpdateTask(ctx, target) {
  ...
  if (type === 'plugin') {
    ...
    const gitSpec = spec.includes('git+') || spec.startsWith('git:')
    const task = newTask('plugin', moduleName)
    const profileDir = join(resolveDshHome(), 'profiles', profile)
    runPnpmAdd(profileDir, gitSpec ? ['add', spec] : ['add', `${moduleName}@latest`], task)
