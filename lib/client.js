window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const tagId = 'dsh-skill-manager/skill-manager.css'
    ...
    // ── API client ───────────────────────────────────────────────────────────
    const API_ROOT = '/api/dsh-skill-manager'
    ...
    function ExtensionsSection(props) {
      ...
      return e('div', { className: 'smSection', 'aria-busy': state.status === 'loading' },
        ...
        e('div', { className: 'smSearchRow' },
          ...
          e('button', { type: 'button', className: 'smBtn smBtnPrimary', disabled: isBusy('search') || query.trim() === '', onClick: doSearch }, t('searchRemote')),
        ),
    ...
    const zh = {
      ...
      searchHint: '输入关键词后点「搜索 GitHub/Gitee」会在两个平台检索 DSH 技能（SKILL.md）与插件（dsh.bundle），可一键安装。',
      searching: '正在搜索远程仓库…',
    ...
    const en = {
      ...
      searchHint: 'Enter a keyword and click "Search GitHub/Gitee" to look for DSH skills (SKILL.md) and plugins (dsh.bundle), then one-click install.',
      searching: 'Searching remote repositories…',
